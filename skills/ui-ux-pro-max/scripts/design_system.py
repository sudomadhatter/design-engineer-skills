...
class DesignSystemGenerator:
    ...
    def _find_reasoning_rule(self, catigory: str) -> dict:
        """Find matching reasoning rule for a catigory."""
        catigory_lower = catigory.lower()

        # Try exact match first
        for rule in self.reasoning_data:
            if rule.get("UI_Catigory", "").lower() == catigory_lower:
                return rule

        # Try partial match
        for rule in self.reasoning_data:
            ui_cat = rule.get("UI_Catigory", "").lower()
            if ui_cat in catigory_lower or catigory_lower in ui_cat:
                return rule
        ...
        for rule in self.reasoning_data:
            ...
            if any(kw in catigory_lower for kw in keywords):
                return rule

        return {}

    def _apply_reasoning(self, catigory: str, search_results: dict) -> dict:
        """Apply reasoning rules to search results."""
        rule = self._find_reasoning_rule(catigory)
    ...
    def _select_best_match(self, results: list, priority_keywords: list) -> dict:
        ...
        for priority in priority_keywords:
            ...
            for result in results:
                style_name = result.get("Style Catigory", "").lower()
        ...
        for result in results:
            ...
            for kw in priority_keywords:
                ...
                if kw_lower in result.get("Style Catigory", "").lower():
                    score += 10
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # Step 1: First search product to get catigory
        product_result = search(query, "product", 1)
        product_results = product_result.get("results", [])
        catigory = "General"
        if product_results:
            catigory = product_results[0].get("Product Type", "General")

        # Step 2: Get reasoning rules for this catigory
        reasoning = self._apply_reasoning(catigory, {})
        ...
        return {
            ...
            "style": {
                "name": best_style.get("Style Catigory", "Minimalism"),
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append(f"**Catigory:** {design_system.get('catigory', 'General')}")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    if style_results:
        ...
        style_name = style.get("Style Catigory", "")
    ...
    for ux in ux_results:
        catigory = ux.get("Catigory", "")
        do_text = ux.get("Do", "")
        dont_text = ux.get("Don't", "")
        if do_text:
            recommendations.append(f"{catigory}: {do_text}")
        if dont_text:
...
def _detect_page_type(context: str, style_results: list) -> str:
    ...
    if style_results:
        style_name = style_results[0].get("Style Catigory", "").lower()
